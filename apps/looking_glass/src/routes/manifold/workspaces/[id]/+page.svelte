<script lang="ts">
  ...
  import { getWorkspaceThoughts, getWorkspaceGraph, getWorkspaceSummary, upsertWorkspaceSummary } from '$lib/api/manifold';
  ...
  async function loadThoughts() {
    try {
      const resp = await getWorkspaceThoughts(workspaceId, false);
  ...
  {:else}
    ...
    {#if activeTab === 'thoughts'}
      ...
      {#if thoughts.length === 0}
        <GlassPanel emptyMessage="No thoughts in this workspace" />
      {/if}
