<script lang="ts">
  ...
  $: typeEntries = Object.entries(session.types || {});
...
<div class="backdrop-blur-glass bg-coalescence-glass border border-coalescence-border rounded-xl p-4 hover:border-indigo-500 transition-all hover:shadow-glow">
  <div class="flex items-center justify-between mb-3">
    <div class="flex-1">
      <div class="text-lg font-semibold text-neutral-100">{session.session_id}</div>
      <div class="text-xs text-neutral-500">{session.count} thoughts</div>
    </div>
    <div class="text-indigo-400 font-semibold">{session.count}</div>
  </div>
