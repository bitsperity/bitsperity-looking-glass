...
const AdminSidebar = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `${open ? ` <button type="button" aria-label="Close admin panel" class="fixed inset-0 bg-black/40 z-40 transition-opacity duration-300"></button>  <div class="fixed right-0 top-0 h-full w-96 bg-neutral-900 border-l border-neutral-800 shadow-2xl z-50 overflow-y-auto animate-slide-in svelte-8gd0xg"><div class="p-6 space-y-6"> <div class="flex items-center justify-between"><h2 class="text-lg font-semibold text-neutral-100" data-svelte-h="svelte-17vaxgz">Admin Panel</h2> <button class="text-neutral-400 hover:text-neutral-200 text-2xl leading-none" data-svelte-h="svelte-bvjgbw">×</button></div>  <div class="space-y-3"><h3 class="text-sm font-medium text-neutral-300" data-svelte-h="svelte-1c3b99d">Vector Store</h3> <div class="bg-neutral-800/50 rounded-lg p-4 space-y-2 border border-neutral-700/50">${embedStatus ? `${isOverallStatus(embedStatus) ? `<div class="flex justify-between text-xs"><span class="text-neutral-400" data-svelte-h="svelte-1ld3gbx">Collection</span> <span class="text-neutral-200 font-mono">${escape(embedStatus.collection_name)}</span></div> <div class="flex justify-between text-xs"><span class="text-neutral-400" data-svelte-h="svelte-1u6rsd">Vectors</span> <span class="text-neutral-200 font-mono">${escape(embedStatus.total_vectors.toLocaleString())}</span></div> <div class="flex justify-between text-xs"><span class="text-neutral-400" data-svelte-h="svelte-1ernmgu">Dimensions</span> <span class="text-neutral-200 font-mono">${escape(embedStatus.vector_size)}D</span></div> <div class="flex justify-between text-xs"><span class="text-neutral-400" data-svelte-h="svelte-1c25w0g">Embedded Articles</span> <span class="text-neutral-200 font-mono">${escape(embedStatus.total_embedded_articles.toLocaleString())}</span></div>` : ``}` : `<div class="text-xs text-neutral-500" data-svelte-h="svelte-ebs8ir">Loading status...</div>`}</div></div>  <div class="space-y-3"><div class="flex items-center justify-between"><h3 class="text-sm font-medium text-neutral-300" data-svelte-h="svelte-aljmeo">Embedding Status</h3> <button class="text-xs text-blue-400 hover:text-blue-300" data-svelte-h="svelte-gm063k">Refresh</button></div> <div class="bg-neutral-800/50 rounded-lg p-4 space-y-3 border border-neutral-700/50">${embedStatus && !isOverallStatus(embedStatus) ? `<div class="flex items-center justify-between"><span class="text-xs text-neutral-400" data-svelte-h="svelte-qn1wtg">Status</span> ${validate_component(Badge, "Badge").$$render(
    $$result,
    {
      variant: embedStatus.status === "done" ? "success" : embedStatus.status === "running" ? "primary" : embedStatus.status === "error" ? "error" : "secondary",
      size: "sm"
  ...
  )}</div> ${embedStatus.status === "running" || embedStatus.status === "done" ? `<div><div class="flex justify-between text-xs mb-1"><span class="text-neutral-400" data-svelte-h="svelte-1g9hxno">Progress</span> <span class="text-neutral-200">${escape(embedStatus.processed.toLocaleString())} / ${escape(embedStatus.total.toLocaleString())}</span></div> <div class="w-full bg-neutral-800 rounded-full h-2 overflow-hidden"><div class="bg-blue-500 h-full transition-all duration-500" style="${"width: " + escape(embedStatus.percent.toFixed(1), true) + "%"}"></div></div> <div class="text-xs text-neutral-500 mt-1">${escape(embedStatus.percent.toFixed(1))}%</div></div>` : ``} ${embedStatus.error ? `<div class="text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded p-2">${escape(embedStatus.error)}</div>` : ``}` : `<div class="text-xs text-neutral-500" data-svelte-h="svelte-168flz">No active job</div>`}</div></div>  <div class="space-y-3"><h3 class="text-sm font-medium text-neutral-300" data-svelte-h="svelte-1nn12y6">Actions</h3> <div class="space-y-2">${validate_component(Button, "Button").$$render(
    $$result,
    {
      variant: "secondary",
      size: "sm",
    ...
    return `<div class="bg-neutral-800/50 border border-neutral-700/50 rounded-lg p-3"><div class="flex items-start justify-between gap-2"><button class="flex-1 text-left hover:bg-neutral-800/50 rounded p-1 -m-1 transition-colors" ${isActiveCollection(col.name) ? "disabled" : ""}><div class="flex items-center gap-2 mb-1"><span class="text-xs font-mono text-neutral-200">${escape(col.name)}</span> ${isActiveCollection(col.name) ? `${validate_component(Badge, "Badge").$$render($$result, { variant: "success", size: "sm" }, {}, {
      default: () => {
        return `Active`;
      }
    })}` : ``}</div> <div class="text-xs text-neutral-400">${escape(col.points_count.toLocaleString())} vectors · ${escape(col.vector_size)}D</div> </button></div> </div>`;
