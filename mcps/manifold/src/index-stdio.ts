...
// Tools
import { getHealthTool, getConfigTool, getDeviceTool } from './lib/tools/health.js';
import { createThoughtTool, getThoughtTool, patchThoughtTool, deleteThoughtTool } from './lib/tools/thoughts.js';
import { searchThoughtsTool, getTimelineTool, getStatsTool } from './lib/tools/search.js';
import { linkRelatedTool, unlinkRelatedTool, getRelatedTool, getRelatedFacetsTool, getRelatedGraphTool } from './lib/tools/relations.js';
import { getBirdviewGraphTool } from './lib/tools/graph.js';
import { promoteThoughtTool, syncAriadneTool } from './lib/tools/promote.js';
import { getHistoryTool, reembedThoughtTool, reindexCollectionTool, dedupeThoughtsTool, bulkQuarantineTool, bulkUnquarantineTool, bulkReembedTool, bulkPromoteTool, getTrashTool, restoreFromTrashTool, getSimilarTool } from './lib/tools/admin.js';
...
logger.info('Manifold MCP tools registered');

const transport = new StdioServerTransport();

async function main() {
  logger.info({ manifoldApiUrl: config.MANIFOLD_API_URL }, 'Starting Manifold MCP (stdio)');
  await server.connect(transport);
  logger.info('Manifold MCP ready');
}

main().catch((error) => {
  logger.error({ error }, 'Failed to start Manifold MCP');
  process.exit(1);
});
